...
**POPL**は **Public Observable Proof Ledger**（公開観測可能証明台帳）の略です。

POPL は、AI やプロトコルの実行結果について、「説明」ではなく「観測された証拠」をもとに記録・参照するための台帳構造です。

本プロジェクトは当初、**Proof of Protocol Ledger** という名称で、
プロトコルレベルの動作証拠に焦点を当てて構想されました。
その後、公開検証・第三者参照・再現性を重視する方向へ進化する中で、
名称も自然に **Public Observable Proof Ledger** へと拡張されました。
